...
`dti` seems to be same as `DTI` except the null values inside `DTI` have been imputed in the `tfd`. 
Let's look insdie `dti`'s `cca`.
...
We can't combine x and y coordinates in `tfd` format :( (x and y are not all unique values).
